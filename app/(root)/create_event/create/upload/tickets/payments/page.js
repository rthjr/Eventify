...
const Payment = () => {
  ...
  const handleNext = async (e) => {
    ...
    if (!isCash && !isQR) {
      ...
      alert("please select please upload ur qr.");
      return;
    } else if (isCash && !isQR) {
      try {
        const existingData = JSON.parse(localStorage.getItem("eventData")) || {};
        const updatedData = { ...existingData, qrUrl: null, isCash: isCash };
      ...
      try {
        ...
        formData.append("upload_preset", "ml_default");
        const existingData = JSON.parse(localStorage.getItem("eventData")) || {};
        const updatedData = { ...existingData, qrUrl, isCash: isCash };
    ...
    <>
      ...
      <div className="w-full h-full flex flex-wrap m-auto ">
        <div className="w-full md:w-5/12 h-auto my-20 m-auto backdrop-blur-2xl  flex flex-col lg:flex-wrap gap-8">
          <div
            action=""
            className="p-4 bg-gray-100 shadow-2xl rounded-lg w-full h-auto flex flex-col gap-8"
          >
            ...
            <div className="w-full p-4 border-2 border-black rounded-lg flex flex-col gap-8">
              <h2 className="text-xl font-bold text-start">
                Please select your payment methods to be linked with this Event.
                Can be Both
              </h2>
              ...
              <div className="w-full h-auto flex flex-wrap justify-end items-end">
                <button
                  className="rounded-lg bg-customPurple-default hover:bg-customPurple-hover text-white p-2"
                  onClick={handleNext}
                >
                  Publish Event
                </button>
